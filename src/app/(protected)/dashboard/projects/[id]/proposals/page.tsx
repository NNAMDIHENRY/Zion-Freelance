...
import {
  getClientProfileIdForUser,
  getProjectOwnedByClient,
...
export default async function ClientProjectProposalsPage({
  ...
  const list = await listProjectProposals(
    session.user.id,
    session.user.role,
    id
  );
  ...
  const rows = list.ok
    ? list.data.map(serializeClientProposalRow)
    : [];
    ...
    <div className="space-y-8">
      <div className="space-y-2">
        ...
        <p className="max-w-2xl text-sm text-muted-foreground">
          Review cover letters, compare pricing, and accept the
          freelancer you want to move forward with.
        </p>
      </div>
      <ClientProjectProposalsTable rows={rows} />
    </div>
  );
}