...
export default async function DashboardProposalsPage() {
  ...
  const list = await listFreelancerProposals(session.user.id, session.user.role);
  const rows = list.ok ? list.data.map(serializeFreelancerProposalRow) : [];
    ...
    <div className="space-y-8">
      <div className="space-y-2">
        ...
        <p className="max-w-2xl text-sm text-muted-foreground">
          Track every bid you have placed, follow client decisions, and withdraw when plans change.
        </p>
      </div>
      ...
      <FreelancerProposalsTable
        ...
        actions={(row) => (
          <ProposalActions proposalId={row.id} status={row.status} role="freelancer" showOpenDetail />
        )}
      />

      <p className="text-sm text-muted-foreground">
        Looking for work?{" "}
        <Link href="/freelancer/jobs" className="font-medium text-primary underline-offset-4 hover:underline">
