...
async function flutterwaveFetch<T>(
  ...
  const base = paymentEnv.flutterwaveBaseUrl ?? "https://api.flutterwave.com/v3";
  const url = `${base}${path}`;
  ...
  await logProviderCall({
    ...
    errorMessage: res.ok ? undefined : (body as { message?: string }).message
  });

  if (!res.ok) {
    throw new Error((body as { message?: string }).message ?? "Flutterwave request failed");
  }
...
export async function initializeFlutterwavePayment(input: InitializePaymentInput) {
  const payload = {
    ...
    customizations: {
      title: "Zion TeCHer",
      description:
        input.meta?.purpose === "ESCROW_FUND" ? "Contract escrow funding" : "Wallet funding"
    },
    meta: input.meta
  };
